"""
Genera prode.html conectado a Google Sheets en tiempo real.
Correr: python gen_sheets.py
"""

SHEET_ID = '1rOE_0gmeYvzFz5xbIQI0fkzdYdhr6YGgaSCNsAsVU2I'
...
.field {{
  ...
  background: linear-gradient(180deg, #1e6c38 0%, #24843c 30%, #1e6c38 60%, #246030 100%);
...
@media (max-width: 768px) {{ .torneos-container {{ grid-template-columns: 1fr; }} }}
...
function getTeamColor(teamName) {{
  const colors = {{
    ...
    'PERCHA FC': {{ bg: '#000000', text: '#FFD700' }}
  }};
...
function renderIdeal() {{
  ...
  const fieldHTML = `<div class="field">${{groups.map(g => `<div class="field-row">${{g.map(p => {{
  ...
  const listHTML = `<div class="ideal-list">${{playersWithTeam.map((p, i) => `<div class="ideal-list-item">
    ...
    <div class="ideal-list-pts">${{p.pts}}</div>
  </div>`).join("")}}</div>`;
